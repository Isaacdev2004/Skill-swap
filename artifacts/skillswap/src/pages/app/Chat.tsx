...
export default function Chat() {
  ...
  const { conversations, messages, activeConversationId, setActiveConversation, sendMessage } = useChatStore();
  const [text, setText] = useState("");
  const scrollRef = useRef<HTMLDivElement>(null);

  // Set active conversation based on route or default to first
  useEffect(() => {
    if (match && params?.id) {
  ...
  const shareMeetLink = () => {
    ...
    sendMessage(activeConversationId, "https://meet.google.com/new-swap-link", "meet-link");
  };
          ...
          <div className="flex-1 overflow-y-auto">
            {conversations.map(conv => (
              <Link key={conv.id} href={`/chat/${conv.id}`}>
                <div 
                  className={`p-4 border-b cursor-pointer transition-colors hover:bg-muted/50 flex items-start gap-3 ${activeConversationId === conv.id ? 'bg-muted' : ''}`}
                >
                  <div className="relative">
                    <Avatar className={conv.participant.avatarColor}>
                      <AvatarFallback className="text-white bg-transparent">{conv.participant.initials}</AvatarFallback>
                    </Avatar>
                    {conv.isOnline && (
          ...
          <div className="flex-1 flex flex-col bg-background relative">
            <div className="h-16 border-b flex items-center justify-between px-6 bg-background/95 backdrop-blur z-10">
              <div className="flex items-center gap-3">
                <Avatar className={activeConv.participant.avatarColor}>
                  <AvatarFallback className="text-white bg-transparent">{activeConv.participant.initials}</AvatarFallback>
                </Avatar>
                <div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6" ref={scrollRef}>
              {activeMsgs.map((msg, index) => {
                ...
                const showAvatar = index === 0 || activeMsgs[index - 1].senderId !== msg.senderId;
                
                return (
                  <div key={msg.id} className={`flex gap-3 max-w-[80%] ${isMe ? 'ml-auto flex-row-reverse' : ''}`}>
                    {showAvatar ? (
                      <Avatar className={`w-8 h-8 shrink-0 ${isMe ? CURRENT_USER.avatarColor : activeConv.participant.avatarColor}`}>
                        <AvatarFallback className="text-white text-xs bg-transparent">
                          {isMe ? CURRENT_USER.initials : activeConv.participant.initials}
                        </AvatarFallback>
                      </Avatar>
                    ) : (
                    ...
                    <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                      {msg.type === 'text' ? (
                        ...
                        <div className={`p-4 rounded-2xl border ${isMe ? 'bg-primary/5 border-primary/20 rounded-tr-sm' : 'bg-muted border-border rounded-tl-sm'}`}>
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">Click to join the video call</p>
                            </div>
                          </div>
                          <a href={msg.text} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline text-xs block mb-3 truncate max-w-[200px]">
                            {msg.text}
                          </a>
            ...
            <div className="p-4 bg-background border-t">
              <form onSubmit={handleSend} className="flex gap-2 items-center">
                <Button type="button" variant="ghost" size="icon" className="text-muted-foreground shrink-0 hidden sm:flex">
                ...
                <div className="relative flex-1">
                  ...
                  />
                  <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1 h-8 w-8 text-muted-foreground">
                    <Smile className="w-5 h-5" />
                  </Button>
                </div>
                <Button type="button" variant="outline" size="icon" onClick={shareMeetLink} className="sm:hidden shrink-0">
